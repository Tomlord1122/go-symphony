...
var (
	...
	postgresDriver = []string{"github.com/jackc/pgx/v5"}
...
// createFrameWorkMap adds Gin framework to Project's FrameworkMap
...
func (p *Project) createDBDriverMap() {
	...
	// Supabase also uses PostgreSQL driver (pgx)
	p.DBDriverMap[flags.Supabase] = Driver{
		packageName: postgresDriver,
		templater:   dbdriver.PostgresTemplate{},
	}
}
...
func (p *Project) createDockerMap() {
	...
	// Supabase can also use Docker for local development
	p.DockerMap[flags.Supabase] = Docker{
		packageName: []string{},
		templater:   docker.PostgresDockerTemplate{},
	}
}
...
func (p *Project) CreateMainFile() error {
	...
	// Install the correct package for the selected framework (Gin)
	...
	if p.AdvancedOptions[string(flags.Sqlc)] && p.DBDriver != flags.Supabase {
		...
		if err != nil {
			log.Printf("Error setting up SQLC: %v", err)
			return err
		}
	}
	...
	// Create .env file (skip for Supabase as it's handled separately)
	if p.DBDriver != flags.Supabase {
...
func (p *Project) CreateSqlcSetup(projectPath string) error {
	sqlcTemplater := sqlc.SqlcTemplate{}

	// Create sqlc.yaml config file
	sqlcConfigFile, err := os.Create(filepath.Join(projectPath, "sqlc.yaml"))
	if err != nil {
		return err
	}
	defer sqlcConfigFile.Close()

	sqlcConfigTemplate := template.Must(template.New("sqlc.yaml").Parse(string(sqlcTemplater.Config())))
	err = sqlcConfigTemplate.Execute(sqlcConfigFile, p)
	if err != nil {
		return err
	}

	// Create sqlc directories
	err = os.MkdirAll(filepath.Join(projectPath, "sqlc", "migrations"), 0755)
	if err != nil {
		return err
	}

	err = os.MkdirAll(filepath.Join(projectPath, "sqlc", "queries"), 0755)
	if err != nil {
		return err
	}

	// Create initial schema migration
	initialSchemaFile, err := os.Create(filepath.Join(projectPath, "sqlc", "migrations", "001_initial_schema.sql"))
	if err != nil {
		return err
	}
	defer initialSchemaFile.Close()

	initialSchemaTemplate := template.Must(template.New("001_initial_schema.sql").Parse(string(sqlcTemplater.InitialSchema())))
	err = initialSchemaTemplate.Execute(initialSchemaFile, p)
	if err != nil {
		return err
	}

	// Create example queries
	usersQueriesFile, err := os.Create(filepath.Join(projectPath, "sqlc", "queries", "users.sql"))
	if err != nil {
		return err
	}
	defer usersQueriesFile.Close()

	usersQueriesTemplate := template.Must(template.New("users.sql").Parse(string(sqlcTemplater.UsersQueries())))
	err = usersQueriesTemplate.Execute(usersQueriesFile, p)
	if err != nil {
		return err
	}

	postsQueriesFile, err := os.Create(filepath.Join(projectPath, "sqlc", "queries", "posts.sql"))
	if err != nil {
		return err
	}
	defer postsQueriesFile.Close()

	postsQueriesTemplate := template.Must(template.New("posts.sql").Parse(string(sqlcTemplater.PostsQueries())))
	err = postsQueriesTemplate.Execute(postsQueriesFile, p)
	if err != nil {
		return err
	}
